...
if ( ! function_exists( 'woo_shortcode_stylesheet' ) ) {
	function woo_shortcode_stylesheet() {
		echo "\n" . "<!-- Woo Shortcodes CSS -->\n";
		echo '<link href="'. plugin_dir_url( __FILE__ ) . 'css/shortcodes.css" rel="stylesheet" type="text/css" />'."\n";
...
function woo_register_shortcode_js () {
	wp_register_script( 'woo-shortcodes', plugin_dir_url( __FILE__ ) . 'js/shortcodes.js', array( 'jquery', 'jquery-ui-tabs' ), '5.0.0' );
...
function woo_shortcode_socialicon ( $atts, $content = null ) {
	...
	if ( $profile_type != '' ) {
		...
		if ( $icon_url ) {
			...
			$_img_url = plugin_dir_path( __FILE__ ) . 'functions/images/ico-social-' . $_profile_to_display . '.png';
		}
	} // End IF Statement
